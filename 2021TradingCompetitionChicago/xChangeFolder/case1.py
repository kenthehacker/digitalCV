...
class PositionTrackerBot(UTCBot):
    ...
    async def place_bids(self, asset):
        ...
        #resp = await self.place_order(asset, pb.OrderSpecType.LIMIT, pb.OrderSpecSide.BID, 1, self.fair[asset] * 0.999)
        #self.bidorderid[asset] = resp.order_id
    ...
    async def place_asks(self, asset):
        ...
        orders = await self.basic_mm(asset, self.fair[asset], self.params["edge"],
            self.params["size"], self.params["limit"],self.max_widths[asset])
        #resp = await self.place_order(asset, pb.OrderSpecType.LIMIT, pb.OrderSpecSide.ASK, 1, self.fair[asset] * 0.9991)
        #self.askorderid[asset] = resp.order_id
    ...
    async def evaluate_fairs(self):
        ...
        #for asset in FUTURES:
            #self.fair[asset] = self.fair[asset] * 0.9998
    ...
    async def basic_mm(self, asset, fair, width, clip, max_pos, max_range):
        ...
        if self.pos['RORUSD'] >= 9 or self.pos['RORUSD'] <= -9 or self.pos[asset] >= 99:
            #print(self.pos)
            bid_s = min(bids_left, clip,0)
            bid_s2 = max(0, min(bids_left - clip, clip,0))
            ask_s = min(asks_left, clip,0)
            ask_s2 = max(0, min(asks_left - clip, clip,0))
