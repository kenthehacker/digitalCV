...
class PositionTrackerBot(UTCBot):
    ...
    async def evaluate_fairs(self):
        ...
        etc. 
        """
        #fair val = cash val * (1 + interest rate * (days to exp / 360))
        print(self.pos)
        #pass
    ...
    async def basic_mm(self, asset, fair, width, clip, max_pos, max_range):
        ...
        if self.pos['RORUSD'] >= 9:
            self.place_order("RORUSD", pb.OrderSpecType.MARKET, pb.OrderSpecSide.ASK, 10)
        elif self.pos['RORUSD'] <= 9:
            self.place_order("RORUSD", pb.OrderSpecType.MARKET, pb.OrderSpecSide.BID, 10)
        elif self.pos[asset] >= 10:
            print('m,nmbn,,asdsadsadaasd')
            self.place_order(asset, pb.OrderSpecType.MARKET, pb.OrderSpecSide.ASK, 10)
        elif self.pos[asset] < 0:
            self.place_order(asset, pb.OrderSpecType.MARKET, pb.OrderSpecSide.BID, 10)
        else:
            print('qweeeee')
            self.place_order(asset, pb.OrderSpecType.MARKET, pb.OrderSpecSide.BID, 1, self.fair[asset] * 0.98)
            self.place_order(asset, pb.OrderSpecType.MARKET, pb.OrderSpecSide.ASK, 1, self.fair[asset] * 0.985)
        return {'asset': asset,
